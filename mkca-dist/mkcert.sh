...
openssl genrsa -out vpnclients/$NAME/$NAME.key $KEYBITS >/dev/null 2>&1
...
openssl req -new -sha256 -key vpnclients/$NAME/$NAME.key \
...
# Sign the request
echo
echo "Signing certificate, enter the CA password"
./sign.sh vpnclients/$NAME/$NAME.csr $EXT $NAME


# Generate pkcs12
echo
echo "Generating PKCS#12"
openssl pkcs12 -export -certfile ca.crt \
